...
int main() {
    ...
    while (!perguntas.empty()) {
        ...
        while (!atualizacoes.empty() && atualizacoes.top().first <= x) {
            ...
            int x1 = atualizacao.first, x2 = atualizacao.second.first,
                y1 = atualizacao.second.second.first,
                y2 = atualizacao.second.second.second;
            update(y1, 1);
            update(y2 + 1, -1);
        }
        while (!remocoes.empty() && remocoes.top().first < x) {
            ...
            remocoes.pop();
            int x2 = remocao.first, y1 = remocao.second.first,
                y2 = remocao.second.second;
            update(y1, -1);
            update(y2 + 1, 1);
        }
        resp[id] = query(y);
