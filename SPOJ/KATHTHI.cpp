...
int main() {
    ...
    while (casos--) {
        ...
        for (int i = 0; i < n; i++)
            for (int j = 0; j < m; j++) visitado[i][j] = false;
        deque<iii> bfs;
        ...
        while (!bfs.empty()) {
            ...
            if (x - 1 >= 0) {
                if (implicito[x][y] == implicito[x - 1][y])
                    bfs.PF(MP(distancia, MP(x - 1, y)));
                else
                    bfs.PB(MP(distancia + 1, MP(x - 1, y)));
            }
            if (x + 1 <= n - 1) {
                if (implicito[x][y] == implicito[x + 1][y])
                    bfs.PF(MP(distancia, MP(x + 1, y)));
            ...
            if (y - 1 >= 0) {
                if (implicito[x][y] == implicito[x][y - 1])
                    bfs.PF(MP(distancia, MP(x, y - 1)));
                else
                    bfs.PB(MP(distancia + 1, MP(x, y - 1)));
            }
            if (y + 1 <= m - 1) {
                if (implicito[x][y] == implicito[x][y + 1])
                    bfs.PF(MP(distancia, MP(x, y + 1)));
