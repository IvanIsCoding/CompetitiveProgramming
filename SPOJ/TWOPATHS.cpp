...
void calcula(int v) {
    ...
    temporario_diametro[v] =
        max(maior_diam[v][0], maior_dist[v][0] + maior_dist[v][1]);
}
...
void dfs2(int v, int p) {
    ...
    for (int u : grafo[v]) {
        ...
        copiav_dist[0] = maior_dist[v][0];
        copiav_dist[1] = maior_dist[v][1];
        copiav_dist[2] = maior_dist[v][2];
        copiau_dist[0] = maior_dist[u][0];
        copiau_dist[1] = maior_dist[u][1];
        copiau_dist[2] = maior_dist[u][2];
        copiav_diam[0] = maior_diam[v][0];
        copiav_diam[1] = maior_diam[v][1];
        copiau_diam[0] = maior_diam[u][0];
        copiau_diam[1] = maior_diam[u][1];
        remove(v, u);
        ll candidato = 1LL * temporario_diametro[v] * temporario_diametro[u];
        melhor_produto = max(melhor_produto, candidato);
        insert(u, v);
        dfs2(u, v);
        maior_dist[v][0] = copiav_dist[0];
        maior_dist[v][1] = copiav_dist[1];
        maior_dist[v][2] = copiav_dist[2];
        maior_dist[u][0] = copiau_dist[0];
        maior_dist[u][1] = copiau_dist[1];
        maior_dist[u][2] = copiau_dist[2];
        maior_diam[v][0] = copiav_diam[0];
        maior_diam[v][1] = copiav_diam[1];
        maior_diam[u][0] = copiau_diam[0];
        maior_diam[u][1] = copiau_diam[1];
        calcula(v);
        calcula(u);
    }
}
