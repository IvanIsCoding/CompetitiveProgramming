...
//	}
//	int mid = (left+right)/2;
//	int sinistra = (esq[pos] == -1) ? 0 :
// query_persistent(esq[pos],left,mid,i,j); 	int destra = (dir[pos] == -1) ?
// 0 : query_persistent(dir[pos],mid+1,right,i,j); 	return sinistra +
// destra;
// }
int kth(int a, int b, int c, int d, int left, int right, int count) {
    // printf("%d %d %d %d %d L %d R %d C %d ",val(a) + val(b) - val(c) -
    // val(d),a,b,c,d,left,right,count);
    if (left == right) {
        return left;
    }
    int mid = (left + right) / 2;
    ...
    // printf("%d\n",davez);
    if (count <= davez)
        return kth(esq[a], esq[b], esq[c], esq[d], left, mid, count);
...
int doQuery(int u, int v, int count) {
    ...
    // ",query_persistent(raiz[u],1,cnt,i,i)); printf("\n"); for(int
    // i=1;i<=cnt;i++) //printf("%d ",query_persistent(raiz[v],1,cnt,i,i));
    // printf("\n");
    // for(int i=1;i<=cnt;i++) //printf("%d
    // ",query_persistent(raiz[ancestor],1,cnt,i,i)); printf("\n"); for(int
    // i=1;i<=cnt;i++) //printf("%d
    // ",query_persistent(raiz[pai[ancestor]],1,cnt,i,i)); printf("\n");
    return reverso[kth(raiz[u], raiz[v], raiz[ancestor], raiz[pai[ancestor]], 1,
                       cnt, count)];
}
int main() {
    scanf("%d %d", &n, &m);
