...
using namespace std;
char entrada[MAXN];
int matriz[MAXN][MAXN], esquerda[MAXN][MAXN], direita[MAXN][MAXN],
    cima[MAXN][MAXN], baixo[MAXN][MAXN], n, TC;
int main() {
    scanf("%d", &TC);
    while (TC--) {
        scanf("%d", &n);
        ...
        for (int i = 1; i <= n; i++) {
            ...
            for (int j = 2; j <= n; j++) {
                esquerda[i][j] =
                    (esquerda[i][j - 1] + matriz[i][j]) * matriz[i][j];
            }
            direita[i][n] = matriz[i][n];
            for (int j = n - 1; j >= 1; j--) {
                direita[i][j] =
                    (direita[i][j + 1] + matriz[i][j]) * matriz[i][j];
            }
            cima[1][i] = matriz[1][i];
            for (int j = 2; j <= n; j++) {
