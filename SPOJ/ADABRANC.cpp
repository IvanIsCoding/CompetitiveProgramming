...
int peso[MAXN], n, m, q;
inline bool compara(ii A, ii B) { return A.first < B.first; }
inline int get_first_pai(int x, int kth) {
    return (*lower_bound(pai[x].rbegin(), pai[x].rend(), ii(kth, 0), compara))
        .second;
}
inline int get_first_tamanho(int x, int kth) {
    return (*lower_bound(tamanho[x].rbegin(), tamanho[x].rend(), ii(kth, 0),
                         compara))
        .second;
}
...
void join(int x, int y, int kth) {
    ...
    if (peso[x] < peso[y]) {
        ...
        tamanho[y].push_back(
            ii(kth, get_first_tamanho(x, kth) + get_first_tamanho(y, kth)));
...
int query(int x, int kth) { return get_first_tamanho(find(x, kth), kth); }
int main() {
    scanf("%d %d %d", &n, &m, &q);
    for (int i = 0; i < n; i++) {
