...
struct node {
    ...
    int query(int left, int right, int i, int j) {
        ...
        int mid = (left + right) / 2;
        if (j <= mid)
            return l ? l->query(left, mid, i, j) : 0;
        else if (i >= mid + 1)
            return r ? r->query(mid + 1, right, i, j) : 0;
        return (l ? l->query(left, mid, i, j) : 0) +
               (r ? r->query(mid + 1, right, i, j) : 0);
    }
};
int vetor[MAXN], n, q;
pnode bit[MAXN];
...
int query(int x1, int x2, int y) { return read(x2, y) - read(x1 - 1, y); }
int main() {
    scanf("%d", &n);
    bit[0] = new node;
    for (int i = 1; i <= n; i++) {
