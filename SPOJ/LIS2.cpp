...
void update1d(int posx, int posy, int val) {
    posy = lower_bound(compbit[posx].begin(), compbit[posx].end(), posy) -
           compbit[posx].begin() + 1;
...
int main() {
    ...
    for (int i = 1; i <= N; i++) {
        X[i] = lower_bound(compressaox.begin(), compressaox.end(), X[i]) -
               compressaox.begin() + 1;
        Y[i] = lower_bound(compressaoy.begin(), compressaoy.end(), Y[i]) -
               compressaoy.begin() + 1;
    ...
    for (int i = 0; i < ordenado.size(); i++) {
        if (get<2>(ordenado[i]) == 0)
            precalcu(get<1>(ordenado[i]), get<0>(ordenado[i]));
        else
            precalcq(get<1>(ordenado[i]), get<0>(ordenado[i]));
    }
