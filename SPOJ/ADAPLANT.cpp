...
int main() {
    ...
    while (TC--) {
        ...
        build(1, 1, n);
        for (int i = 1; i <= n; i++) {
            int menor = min(query(1, 1, n, max(1, i - k - 1), max(i - 1, 1)),
                            query(1, 1, n, min(i + 1, n), min(i + k + 1, n)));
            // printf("Menor %d Q1 %d Q2 %d\n",menor,query(1,1,n,max(1,i - k -
            // 1),max(i-1,1)),query(1,1,n,min(i+1,n),min(i+k+1,n)));
