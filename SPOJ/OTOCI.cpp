...
char entrada[200];
vector<iii> queries;
int dfsNum, chainNum, chainPtr, vetor[MAXN], nivel[MAXN], chainId[MAXN],
    chainHead[MAXN], chainPos[MAXN], bit[MAXN], pai[MAXN], tamanho[MAXN],
    cor[MAXN], n, m, dsu[MAXN], peso[MAXN], processado[MAXN];
...
int query_bit(int a, int b) { return read_bit(b) - read_bit(a - 1); }
...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        int op = queries[i].first, u = queries[i].second.first,
