...
int query(int pos, int left, int right, int i, int j) {
    ...
    if (j <= mid) {
        ...
        return max(query(2 * pos + 1, left, mid, i, j),
                   query(2 * pos + 2, mid + 1, right, i, j));
    }
}
int main() {
    scanf("%d %d", &n, &m);
    ...
    for (int i = 0; i < m; i++) {
        int h = get<0>(queries[i]), a = get<1>(queries[i]),
            b = get<2>(queries[i]), idx = get<3>(queries[i]);
        while (ptr < n && atualizacoes[ptr].first <= h) {
            update(0, 0, n - 1, atualizacoes[ptr].second,
                   atualizacoes[ptr].first);
            ptr++;
        }
