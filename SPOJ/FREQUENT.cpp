...
#define MAXN 100001
using namespace std;
int n, q, sqn, frequencia[MAXN], conjunto[MAXN], vetor[MAXN], maximo, mo_left,
    mo_right, resposta[MAXN], contador;
...
int main() {
    while (scanf("%d", &n) && n) {
        ...
        for (set<int>::iterator it = valores.begin(); it != valores.end();
             it++) {
