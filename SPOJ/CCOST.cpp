...
inline int query(int a, int b) { return read(b) - read(a - 1); }
int main() {
    int TC;
    getint(TC);
    while (TC--) {
        ...
        for (int i = 0; i < sweep.size(); i++) {
            ...
            while (ptr < n && pontos[ptr].first <= x) {
                int y = lower_bound(compressao.begin(), compressao.end(),
                                    pontos[ptr].second.first) -
                        compressao.begin() + 1;
            ...
            if (r < compressao[0] || l > compressao.back()) {
                ...
                l = lower_bound(compressao.begin(), compressao.end(), l) -
                    compressao.begin() + 1;
                r = prev(upper_bound(compressao.begin(), compressao.end(), r)) -
                    compressao.begin() + 1;
                ans = query(l, r);
            }
            if (idx < 0)
