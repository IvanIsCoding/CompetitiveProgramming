...
void build(int pos, int left, int right) {
    if (left == right) {
        arvore[pos].maximo = arvore[pos].maximo_ini = arvore[pos].maximo_fim =
            arvore[pos].total = vetor[left];
        return;
    }
...
int range_sum(int a, int b) { return acumulada[b] - acumulada[a - 1]; }
int solve(int x1, int y1, int x2, int y2) {
    // printf("Solve\n");
    if (y1 < x2) {
        return query(1, 1, n, x1, y1).maximo_fim + range_sum(y1 + 1, x2 - 1) +
               query(1, 1, n, x2, y2).maximo_ini;
    }
    // printf("Passou\n");
    int ans = query(1, 1, n, x2, y1).maximo;
    if (x2 - 1 >= x1)
        ans = max(ans, query(1, 1, n, x1, x2 - 1).maximo_fim +
                           query(1, 1, n, x2, y2).maximo_ini);
    if (y1 + 1 <= y2)
        ans = max(ans, query(1, 1, n, x1, y1).maximo_fim +
                           query(1, 1, n, y1 + 1, y2).maximo_ini);
    return ans;
}
int main() {
    int TC;
