...
void update(long long novo, long long velho, long long left, long long right,
            long long x, long long delta) {
...
long long query(long long pos, long long left, long long right, long long i,
                long long j) {
    if (left >= i && right <= j) return val[pos];
    long long temp = (1LL * left + 1LL * right) / 2LL;
    long long meio = temp;
    if (j <= meio)
        return query(esq[pos], left, meio, i, j);
    else if (i >= meio + 1)
        return query(dir[pos], meio + 1, right, i, j);
    return query(esq[pos], left, meio, i, j) +
           query(dir[pos], meio + 1, right, i, j);
}
int main() {
    ...
    while (scanf("%lld", &N) && N > 0) {
        ...
        while (Q--) {
            ...
            while (ini <= fim) {
                ...
                long long meio = temp;
                long long lo =
                    lower_bound(compx.begin(), compx.end(), x - meio) -
                    compx.begin() + 1;
                long long hi =
                    prev(upper_bound(compx.begin(), compx.end(), x + meio)) -
                    compx.begin() + 1;
                long long y_l = max(1LL * y - temp, 1LL);
                long long y_h = min(1LL * y + temp, 1LL * MAXN);
                if (query(raiz[hi], 1, MAXN, y_l, y_h) -
                        query(raiz[lo - 1], 1, MAXN, y_l, y_h) >=
                    1) {
                    resp = meio;
                    fim = meio - 1;
                } else {
