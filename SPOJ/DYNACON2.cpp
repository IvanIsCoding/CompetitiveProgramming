...
struct edge {
    ...
    bool contido(int a, int b) const { return a >= l && b <= r; }
};
...
char entrada[50];
vector<edge> A;
int pai[MAXN], tamanho[MAXN], oldx[MAXN], oldy[MAXN], oldtamx[MAXN],
    oldtamy[MAXN], ehatualizacao[MAXN], dsuPtr, N, M;
...
void solve(int left, int right, vector<edge> E) {
    ...
    for (edge davez : E) {
        if (davez.contido(left, right)) {
            ...
            // printf("Adicionei L %d R %d U %d V
    ...
    if (left == right) {
        if (e3[left] == -1)
            ...
            e3[left] = (find(e1[left]) == find(e2[left]));
        // return;
    } else {
        solve(left, mid, L);
        solve(mid + 1, right, R);
...
int main() {
    ...
    for (map<ii, int>::iterator it = arestas.begin(); it != arestas.end();
         it++) {
        ii davez = (*it).first;
        int ini = (*it).second;
    ...
    // for(edge davez : A) printf("L %d R %d U %d V
    // %d\n",davez.l,davez.r,davez.u,davez.v);
