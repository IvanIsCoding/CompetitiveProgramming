...
int dfsNum, chainNum, chainPtr, vetor[MAXN], chainId[MAXN], chainPos[MAXN],
    chainHead[MAXN], tamanho[MAXN], nivel[MAXN], chainVal[MAXN], pai[MAXN],
    numero[MAXN], reversePos[MAXN], n, q;
...
void build_kadane(int pos, int left, int right) {
    ...
    if (left == right) {
        ...
        kadane[pos].maximo_ini = kadane[pos].maximo_fim = kadane[pos].maximo =
            max(kadane[pos].total, 0);
        return;
    }
...
no query_kadane(int pos, int left, int right, int i, int j) {
    ...
    int mid = (left + right) / 2;
    return join(query_kadane(2 * pos, left, mid, i, j),
                query_kadane(2 * pos + 1, mid + 1, right, i, j));
}
...
ii query_lca(int pos, int left, int right, int i, int j) {
    ...
    int mid = (left + right) / 2;
    return min(query_lca(2 * pos, left, mid, i, j),
               query_lca(2 * pos + 1, mid + 1, right, i, j));
}
int LCA(int u, int v) {
    // printf("LCA\n");
    return query_lca(1, 1, dfsNum, min(vetor[u], vetor[v]),
                     max(vetor[u], vetor[v]))
        .second;
}
...
void updateup(int u, int v, int c) {
    ...
    while (true) {
        ...
        update_kadane(1, 1, chainPtr, chainPos[chainHead[uchain]], chainPos[u],
                      c);
        u = chainHead[uchain];
        u = pai[u];
    }
...
int doQuery(int u, int v) {
    ...
    for (int i = 0; i < primeiro.size(); i++)
        resp.push_back(MP(primeiro[i].second, primeiro[i].first));
    ...
    no resultado;
    resultado.maximo_fim = resultado.maximo_ini = resultado.maximo =
        resultado.total = 0;
    for (int i = 0; i < resp.size(); i++) {
        ...
        if (chainPos[l] <= chainPos[r]) {
            ...
            resultado = join(resultado, query_kadane(1, 1, chainPtr,
                                                     chainPos[l], chainPos[r]));
        } else {
            // printf("L %d R %d MAX
            // %d\n",l,r,troca(query_kadane(1,1,chainPtr,chainPos[r],chainPos[l])).maximo);
            resultado = join(
                resultado,
                troca(query_kadane(1, 1, chainPtr, chainPos[r], chainPos[l])));
        }
    }
...
int main() {
    ...
    // for(int i=1;i<=n;i++){
    // printf("I %d Id %d Head %d Pos %d Pai
