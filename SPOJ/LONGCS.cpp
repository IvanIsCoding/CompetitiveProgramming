...
ll hash1[MAXK][MAXN], pot1[MAXN], invpot1[MAXN], hash2[MAXK][MAXN], pot2[MAXN],
    invpot2[MAXN];
int tamanhos[MAXK], K;
...
int check(int sz) {
    ...
    for (int k = 1; k < K && !truque[1].empty(); k++) {
        ...
        truque[1].erase(unique(truque[1].begin(), truque[1].end()),
                        truque[1].end());
        ...
        for (int i = 1, j = sz; i <= tamanhos[k]; i++, j++) {
            ...
            if (binary_search(truque[0].begin(), truque[0].end(), davez))
                truque[1].push_back(davez);
        }
    }
    ...
    while (TC--) {
        ...
        for (int k = 0; k < K; k++) {
            ...
            for (int i = 1; i <= tamanhos[k]; i++) {
                hash1[k][i] =
                    (hash1[k][i - 1] + entrada[k][i - 1] * pot1[i]) % MOD1;
                hash2[k][i] =
                    (hash2[k][i - 1] + entrada[k][i - 1] * pot2[i]) % MOD2;
            }
        }
