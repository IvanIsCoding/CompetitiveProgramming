...
#define LSOne(S) (S & (-S))
using namespace std;
int vetor[MAXN], bit[MAXN][MAXS], contador, n, S, q, entrada2[MAXN],
    entrada3[MAXN], entrada4[MAXN];
char entrada1[MAXN];
set<int> conjunto;
...
int main() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (entrada1[i] == 'C') {
            int resposta = 0;
            int c_left = entrada2[i] / S, c_right = entrada3[i] / S,
                left = entrada2[i], right = entrada3[i],
                x = compressao[entrada4[i]];
            if (c_left == c_right) {
                ...
                for (int j = c_left + 1; j <= c_right - 1; j++)
                    resposta += sum(x, j);
