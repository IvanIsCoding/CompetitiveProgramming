...
int solve(int pos, int oxygen, int nitrogen) {
    oxygen = max(oxygen, 0);
    nitrogen = max(nitrogen, 0);
    if (oxygen == 0 && nitrogen == 0) return 0;
    if (pos > n) {
        return INF;
    }
    if (dp[pos][oxygen][nitrogen] != -1) return dp[pos][oxygen][nitrogen];
    return dp[pos][oxygen][nitrogen] = min(
               solve(pos + 1, oxygen - v1[pos], nitrogen - v2[pos]) + cost[pos],
               solve(pos + 1, oxygen, nitrogen));
}
int main() {
    int TC;
    scanf("%d", &TC);
