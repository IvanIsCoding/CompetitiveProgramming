...
ll hash1[MAXN][MAXN], pot1[MAXN], invpot1[MAXN], hash2[MAXN][MAXN], pot2[MAXN],
    invpot2[MAXN], hash3[MAXN][MAXN], pot3[MAXN], invpot3[MAXN];
...
inline int igualdade(int p1, int ini1, int fim1, int p2, int ini2, int fim2) {
    ...
    if (get_hash1(p1, ini1, fim1) != get_hash1(p2, ini2, fim2)) {
        // if(flag) printf("Falso negativo %d %d %d %d %d %d
    ...
    // if(flag) printf("Falso negativo %d %d %d %d %d %d
    // hash3\n",p1,ini1,fim1,p2,ini2,fim2); return 0;
    //}
    // if(!flag){
    // printf("Falso positivo %d %d %d %d %d %d\n",p1,ini1,fim1,p2,ini2,fim2);
    // printf("%lld %lld
    // hash1\n",get_hash1(p1,ini1,fim1),get_hash1(p2,ini2,fim2)); printf("%lld
...
bool comp(int p1, int p2) {
    if (vetor[p1][inicio[p1]] != vetor[p2][inicio[p2]])
        return vetor[p1][inicio[p1]] < vetor[p2][inicio[p2]];
    ...
    for (int i = logaritmo2[tam]; i >= 0; i--) {
        int novo = atual + (1 << i);
        if (novo <= tam && igualdade(p1, inicio[p1], inicio[p1] + novo - 1, p2,
                                     inicio[p2], inicio[p2] + novo - 1)) {
            atual = novo;
        }
    }
...
inline bool heap_comp(int p1, int p2) { return !comp(p1, p2); }
int main() {
    cin.tie(0);
    cout.tie(0);
    ...
    conversao.erase(unique(conversao.begin(), conversao.end()),
                    conversao.end());
    for (int i = 1; i <= N; i++) {
        for (int j = 1; j <= tamanho[i]; j++) {
            vetor[i][j] =
                lower_bound(conversao.begin(), conversao.end(), vetor[i][j]) -
                conversao.begin() + 1;
