...
int main() {
    ...
    while (TC--) {
        ...
        while (!bfs.empty()) {
            iii davez = bfs.front();
            bfs.pop_front();
            int oldbitmask = davez.first, x = davez.second.first,
            ...
            if (x + 1 < n && !visitado[x + 1][y][bitmask]) {
                if (matriz[x][y] == matriz[x + 1][y])
                    bfs.push_front(MP(bitmask, MP(x + 1, y)));
                else
                    bfs.push_back(MP(bitmask, MP(x + 1, y)));
            }
            if (y + 1 < n && !visitado[x][y + 1][bitmask]) {
                if (matriz[x][y] == matriz[x][y + 1])
                    bfs.push_front(MP(bitmask, MP(x, y + 1)));
                else
                    bfs.push_back(MP(bitmask, MP(x, y + 1)));
            }
