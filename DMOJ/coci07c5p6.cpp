...
int main() {
    ...
    for (int i = 1; i <= Q; i++) {
        cin >> s2[i];
        s2[i].push_back('z' + 1);
        if (apareceu.count(s2[i]))
            ordenado.push_back(ii(apareceu[s2[i]], i));
        else
            ordenado.push_back(ii(N, i));
    }
    ...
    for (int lo = 1; lo <= N; lo += balde) {
        ...
        for (int sweep = 0; sweep < Q; sweep++) {
            int atualdavez = ordenado[sweep].first,
                idx = ordenado[sweep].second;
            ...
            for (int i = 0; i < s2[idx].size(); i++) {
                char c = s2[idx][i] - 'a';
                if (Trie[atual][c]) {
                    // cout << c << " " << freq[atual] - freq[Trie[atual][c]] <<
                    // endl;
