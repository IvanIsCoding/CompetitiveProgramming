...
inline int H(int x, int y) { return abs(x - alvox) + abs(y - alvoy); }
inline int valido(int x, int y) {
    return x >= 0 && y >= 0 && x < n && y < m && entrada[x][y] != 'X' &&
           !processado[x][y];
}
int main() {
    ...
    while (TC--) {
        ...
        while (!Astar.empty()) {
            ...
            Astar.pop();
            int heuristica = get<0>(davez), dist = get<1>(davez),
                x = get<2>(davez), y = get<3>(davez);
            if (x == alvox && y == alvoy) {
                cout << dist << '\n';
                possivel = 1;
                break;
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (valido(nx, ny))
                    Astar.push(
                        make_tuple(H(nx, ny) + dist + 1, dist + 1, nx, ny));
            }
        }
