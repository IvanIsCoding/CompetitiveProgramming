...
ll solve(int p1, int p2, int lado) {
    ...
    if (lado == 0) {
        ll opt1 = solve(p1 + 1, p2, 0) +
                  abs(A[p1 + 1] - A[p1]) * ((n - p1) + (m - p2));
        ll opt2 = solve(p1, p2 + 1, 1) +
                  abs(B[p2 + 1] - A[p1]) * ((n - p1) + (m - p2));
        return dp[p1][p2][lado] = min(opt1, opt2);
    } else {
        ll opt1 = solve(p1 + 1, p2, 0) +
                  abs(A[p1 + 1] - B[p2]) * ((n - p1) + (m - p2));
        ll opt2 = solve(p1, p2 + 1, 1) +
                  abs(B[p2 + 1] - B[p2]) * ((n - p1) + (m - p2));
        return dp[p1][p2][lado] = min(opt1, opt2);
    }
}
...
ll testa(int qtd1, int qtd2) {
    ...
    m = qtd2;
    iteracao++;
    return memo[qtd1][qtd2] =
               (qtd1 + qtd2) * T - min(solve(0, 0, 0), solve(0, 0, 1));
}

bool compara(ll a, ll b) { return a > b; }
