...
ll soma(ll x1, ll y1, ll x2, ll y2) {
    return acumulado[x2][y2] - acumulado[x2][y1 - 1] - acumulado[x1 - 1][y2] +
           acumulado[x1 - 1][y1 - 1];
}
ll solve(ll x1, ll y1, ll x2, ll y2) {
    ...
    for (ll quebra = x1; quebra + 1 <= x2; quebra++) {
        ans =
            min(ans, solve(x1, y1, quebra, y2) + solve(quebra + 1, y1, x2, y2) +
                         soma(x1, y1, x2, y2));
    }
    for (ll quebra = y1; quebra + 1 <= y2; quebra++) {
        ans =
            min(ans, solve(x1, y1, x2, quebra) + solve(x1, quebra + 1, x2, y2) +
                         soma(x1, y1, x2, y2));
    }
...
int main() {
    ...
    for (ll i = 1; i <= n; i++) {
        for (ll j = 1; j <= m; j++) {
            ...
            acumulado[i][j] = matriz[i][j] + acumulado[i - 1][j] +
                              acumulado[i][j - 1] - acumulado[i - 1][j - 1];
        }
    }
    printf("%lld\n", solve(1, 1, n, m));
    return 0;
