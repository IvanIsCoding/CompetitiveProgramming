...
int main() {
    ...
    for (int i = 1; i <= N; i++) {
        H1[i] = lower_bound(compressao.begin(), compressao.end(), H1[i]) -
                compressao.begin();
    }

    for (int i = 1; i <= M; i++) {
        H2[i] = lower_bound(compressao.begin(), compressao.end(), H2[i]) -
                compressao.begin();
    }
    ...
    for (int i = 0; i <= compressao.size(); i++)
        sort(possiveis[i].begin(), possiveis[i].end());
    ...
    for (int bitmask = 1; bitmask < (1 << M); bitmask++) {
        ...
        for (int i = 0; i <= H2[first]; i++) {
            resposta +=
                possiveis[i].end() - lower_bound(possiveis[i].begin(),
                                                 possiveis[i].end(), K - total);
        }
    }
