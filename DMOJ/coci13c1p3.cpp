...
int calcula(int x1, int y1, int x2, int y2) {
    return soma[x2][y2] - soma[x1 - 1][y2] - soma[x2][y1 - 1] +
           soma[x1 - 1][y1 - 1];
}
        ...
        for (int j = y; j <= n; j++) {
            ...
            if (i + 1 <= n && j + 1 <= n) {
                vector<int>::iterator it = lower_bound(
                    mapa[i + 1][j + 1].begin(), mapa[i + 1][j + 1].end(), val);
                if (it != mapa[i + 1][j + 1].end() && (*it) == val)
                    resp += upper_bound(mapa[i + 1][j + 1].begin(),
                                        mapa[i + 1][j + 1].end(), val) -
                            it;
            }
        }
    }
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= n; j++) {
            ...
            soma[i][j] = matriz[i][j] + soma[i - 1][j] + soma[i][j - 1] -
                         soma[i - 1][j - 1];
        }
    }
    for (int i = 1; i <= n; i++) {
