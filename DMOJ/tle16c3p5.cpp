...
inline int custo(int i, int j) { return linear[j] - linear[i - 1]; }
inline rectangle solve(int l) {
    for (int ini = 1, fim = l; fim <= C; ini++, fim++) {
        for (int i = 1; i <= R; i++)
            linear[i] =
                ((soma[i][fim] - soma[i][ini - 1]) == l) + linear[i - 1];
        for (int outroini = 1, outrofim = l; outrofim <= R;
             outroini++, outrofim++) {
...
inline int in_rectangle(int x, int y) {
    return atual.first.first <= x && x <= atual.second.first &&
           atual.first.second <= y && y <= atual.second.second;
}
inline void bs() {
