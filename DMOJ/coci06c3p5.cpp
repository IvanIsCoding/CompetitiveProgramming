...
const int MOD = 1e9;
ii dp[MAXN];
int n, m, componente[MAXN], tam[MAXN], compId, processado[MAXN], flag,
    chegano2[MAXN];
...
void chega2(int x) {
    ...
    chegano2[x] = 1;
    for (int v : transposto[x])
        if (!processado[v]) chega2(v);
}
void checa(int x) {
    processado[x] = 1;
    if (tam[componente[x]] != 1 && chegano2[x]) flag = 1;
    for (int v : grafo[x])
        if (!processado[v]) checa(v);
}
ii solve(int v) {
