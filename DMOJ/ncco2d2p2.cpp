...
double resp;
int n;
inline ll sq(int p) { return 1LL * p * p; }
void solve(ii A, ii B, ii C, ii D) {
    ...
    if (cords[0] != cords[1] || cords[2] != cords[3] || cords[4] != cords[5])
        return;
...
void calculate() {
    for (int i = 0; i < temporario.size(); i++) {
        for (int j = i + 1; j < temporario.size(); j++) {
            solve(temporario[i].F, temporario[i].S, temporario[j].F,
                  temporario[j].S);
        }
    }
    temporario.clear();
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) {
            tripla davez = make_tuple(
                pontos[i].F + pontos[j].F, pontos[i].S + pontos[j].S,
                sq(pontos[i].F - pontos[j].F) + sq(pontos[i].S - pontos[j].S));
