...
struct node {
    ...
    int prior, key;
    ll pure, val;
    node(int _key, ll _pure)
        : l(NULL), r(NULL), prior(rand()), key(_key), pure(_pure), val(_pure) {}
};
    ...
    ll query(int left, int right, int i, int j, int p, int q) {
        ...
        if (j <= mid) {
            ...
            if (t1 == 1LL)
                ...
                return gcd2(t1, (r == NULL)
                                    ? 0LL
                                    : r->query(mid + 1, right, i, j, p, q));
        }
    }
};
pseg raiz;
...
void update(int x, int y, ll delta) { raiz->update(0, R - 1, x, y, delta); }
