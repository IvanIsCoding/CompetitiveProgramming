...
inline short calcula(int id, int x1, int y1, int x2, int y2) {
    return soma[id][x2][y2] - soma[id][x2][y1 - 1] - soma[id][x1 - 1][y2] +
           soma[id][x1 - 1][y1 - 1];
}
...
inline void solve() {
    for (int x = 1; x <= N; x++) {
        for (int y = 1; y <= N; y++) {
            ...
            int minimo = resp / N;
            for (int i = 1; i <= N; i++) {
                for (int tam = minimo; x + tam - 1 <= N && y + tam - 1 <= N;
                     tam++) {
                    if (calcula(i, x, y, x + tam - 1, y + tam - 1) ==
                        tam * tam) {
                        linear[i] = tam;
                    } else
                        break;
...
int main() {
    ...
    for (int k = 1; k <= N; k++) {
        for (int i = 1; i <= N; i++) {
            for (int j = 1; j <= N; j++) {
                soma[k][i][j] = matriz[k][i][j] + soma[k][i][j - 1] +
                                soma[k][i - 1][j] - soma[k][i - 1][j - 1];
            }
        }
    }
    solve();
    for (int k = 1; k <= N; k++) {
        for (int i = 1; i <= N; i++) {
            for (int j = 1; j <= N; j++) {
                ...
                soma[k][i][j] = copia[k][i][j] + soma[k][i][j - 1] +
                                soma[k][i - 1][j] - soma[k][i - 1][j - 1];
            }
        }
    }
