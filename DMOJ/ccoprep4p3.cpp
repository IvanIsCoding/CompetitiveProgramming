...
ll pai[MAXN], peso[MAXN], lazypai[MAXN], lazypeso[MAXN], ultima[MAXN], n, m, q,
    e1[MAXM], e2[MAXM], custo, iteracao;
void zerar() {
    for (ll i = 1; i <= n; i++) pai[i] = i, peso[i] = 1;
}
ll find(ll x) {
    if (x == pai[x]) return x;
...
int main() {
    ...
    for (ll i = 1; i <= m; i++) {
        for (ll j = 0; j + 1 < periodos[i].size(); j++) {
            arestas.push_back(make_tuple(periodos[i][j].first, e1[i], e2[i],
                                         periodos[i][j].second,
                                         periodos[i][j + 1].second - 1));
    ...
    for (ll balde_davez = 0; balde_davez <= tot_baldes; balde_davez++) {
        ...
        for (ll i = 0; i < arestas.size(); i++) {
            ll u = get<1>(arestas[i]), v = get<2>(arestas[i]),
               w = get<0>(arestas[i]), t1 = get<3>(arestas[i]),
               t2 = get<4>(arestas[i]);
        ...
        for (ll tempo = lo; tempo <= hi; tempo++) {
            ...
            for (ll i = 0; i < especial.size(); i++) {
                ll u = get<1>(especial[i]), v = get<2>(especial[i]),
                   w = get<0>(especial[i]), t1 = get<3>(especial[i]),
                   t2 = get<4>(especial[i]);
