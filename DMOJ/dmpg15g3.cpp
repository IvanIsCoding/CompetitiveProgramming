...
int N, LK, RK, LC, RC;
int tam[MAXN], block[MAXN], kinako[MAXN], croissant[MAXN], bit[MAXN],
    tamanho_componente;
int nivel[MAXN], pai[MAXN], linearizacao[MAXN], ponteiroDfs, lin_c[MAXN],
    lin_f[MAXN];
char pao[MAXN];
...
long long sweepline(vector<ii> pares, int fatorc, int fatork) {
    ...
    for (int i = 0; i < pares.size(); i++) {
        ...
        if (checa_intervalo(par1.first, par1.second, lo_c, hi_c, lo_k, hi_k))
            contou--;
    ...
    while (i >= 0 || j >= 0) {
        ...
        int x = get<0>(sweep), a = get<1>(sweep), b = get<2>(sweep),
            c = get<3>(sweep);
...
int main() {
    ...
    for (int i = 1; i <= N; i++) {
        ...
        kinako[i] = (pao[i - 1] == 'K');
        if (LK <= kinako[i] && kinako[i] <= RK && LC <= croissant[i] &&
            croissant[i] <= RC)
            total_pares++;
    }
    for (int i = 1; i < N; i++) {
