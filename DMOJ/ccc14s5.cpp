...
inline int abs(ii davez) {
    return int(davez.first) * int(davez.first) +
           int(davez.second) * int(davez.second);
}
inline int hypot(ii A, ii B) {
    return abs(ii(A.first - B.first, A.second - B.second));
}
inline int get_v(int i, int val) {
    return prev(upper_bound(comp[i].begin(), comp[i].end(), val)) -
           comp[i].begin() + 1;
}
int main() {
    scanf("%d", &n);
    ...
    for (int idx = 0; idx < ordenado.size(); idx++) {
        int dist = get<0>(ordenado[idx]), u = get<1>(ordenado[idx]),
            v = get<2>(ordenado[idx]);
    ...
    for (int i = 2; i <= n; i++)
        resp = max(resp, raiz[i].query(get_v(i, modulo[i] - 1)));
    printf("%d\n", resp);
    return 0;
}
0;
