...
inline ll sq(ll a) { return mul(a, a); }
...
ll inv(ll z) { return fast_expo(z, MOD - 2); }
ll matriz[MAXN][MAXN];
int n;
void exibe() {
    for (int i = 1; i <= n; i++) {
    ...
    for (int linha = 1; linha <= n - 1; linha++) {
        if (matriz[linha][linha] == 0) {
            ...
            for (int coluna = 1; coluna <= n; coluna++)
                matriz[linha][coluna] =
                    add(matriz[linha][coluna], matriz[achou][coluna]);
        }
        ll divisor = inv(matriz[linha][linha]);
        fora = mul(fora, matriz[linha][linha]);
        // printf("F %lld D %lld\n",fora,divisor);
        for (int coluna = linha; coluna <= n; coluna++)
            matriz[linha][coluna] = mul(matriz[linha][coluna], divisor);
        // exibe();
        for (int proxlinha = linha + 1; proxlinha <= n; proxlinha++) {
            ll cof = matriz[proxlinha][linha];
            for (int coluna = 1; coluna <= n; coluna++)
                matriz[proxlinha][coluna] =
                    sub(matriz[proxlinha][coluna], cof * matriz[linha][coluna]);
        }
        // exibe();
    }
