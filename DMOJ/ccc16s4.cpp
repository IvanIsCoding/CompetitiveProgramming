...
int calcula(int ini, int fim) { return soma[fim] - soma[ini - 1]; }
int solve(int ini, int fim) {
    ...
    for (int quebra = ini; quebra + 1 <= fim; quebra++) {
        if (calcula(ini, quebra) == calcula(quebra + 1, fim) &&
            solve(ini, quebra) && solve(quebra + 1, fim)) {
            return dp[ini][fim] = 1;
        }
    }
    for (int quebra = ini; quebra <= fim; quebra++) {
        for (int comecaoutro = quebra + 2; comecaoutro <= fim; comecaoutro++) {
            if (calcula(ini, quebra) == calcula(comecaoutro, fim) &&
                solve(ini, quebra) && solve(comecaoutro, fim)) {
