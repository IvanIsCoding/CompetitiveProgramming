...
long long solve(int Q) {
    ...
    compressao.erase(unique(compressao.begin(), compressao.end()),
                     compressao.end());
    sort(sweep.begin(), sweep.end());
    for (int i = 0; i < sweep.size(); i++) {
        int x = get<0>(sweep[i]), delta = get<1>(sweep[i]),
            y1 = get<2>(sweep[i]), y2 = get<3>(sweep[i]);
        resp += 1LL * (x - lastx) * soma[1];
        y1 = lower_bound(compressao.begin(), compressao.end(), y1) -
             compressao.begin();
        y2 = lower_bound(compressao.begin(), compressao.end(), y2) -
             compressao.begin();
        // printf("Y %d\n",compressao[y2] - compressao[y1]);
        if (delta == 1)
            add(1, 0, compressao.size() + 1, y1, y2 - 1);
        else
            remove(1, 0, compressao.size() + 1, y1, y2 - 1);
        lastx = x;
    }
    return resp;
}
