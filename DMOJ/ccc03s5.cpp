...
int parent[MAXN], pai[MAXN], superpai[MAXN], ligapai[MAXN], ligasuperpai[MAXN],
    nivel[MAXN];
int find(int x) {
...
int main() {
    ...
    for (int i = m - 1; i >= 0; i--) {
        if (find(Kruskal[i].second.first) != find(Kruskal[i].second.second)) {
            ...
            grafo[Kruskal[i].second.first].push_back(
                MP(Kruskal[i].second.second, Kruskal[i].first));
            grafo[Kruskal[i].second.second].push_back(
                MP(Kruskal[i].second.first, Kruskal[i].first));
        }
    }
    pai[1] = 1;
    dfs_pai(1);
