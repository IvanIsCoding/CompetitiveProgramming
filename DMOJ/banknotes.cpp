    ...
    for (int i = 1; i <= N; i++) {
        ...
        for (int resto = 0; resto < b[i]; resto++) {
            ...
            for (int j = 1; j < c[i] && ultimo - j * b[i] >= 0; j++) {
                ii novo =
                    ii(dp[i - 1][ultimo - j * b[i]] + j, ultimo - j * b[i]);
                while (!janela.empty() && janela.back().first >= novo.first)
                    janela.pop_back();
                janela.push_back(novo);
            }
            for (int j = 0, val = ultimo; val > 0; j++, val -= b[i]) {
                while (!janela.empty() && janela.front().second >= val)
                    janela.pop_front();
                if (val - c[i] * b[i] >= 0) {
                    ii novo = ii(dp[i - 1][val - c[i] * b[i]] + j + c[i],
                                 val - c[i] * b[i]);
                    while (!janela.empty() && janela.back().first >= novo.first)
                        janela.pop_back();
                    janela.push_back(novo);
                ...
                if (!janela.empty())
                    dp[i][val] = min(dp[i][val], janela.front().first - j);
                // printf("I %d Resto %d Val %d Dp %d
