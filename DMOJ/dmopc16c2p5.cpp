...
int main() {
    ...
    val.push_back(0);
    raiz.push_back(0);
    invertido.push_back(0);
    janela.push_back(ii(0, 0));
    for (int vez = 1; vez <= M; vez++) {
        ...
        if (op == 'F') {
            int p;
            scanf("%d", &p);
            raiz.push_back(++segIt);
            esq.push_back(0);
            dir.push_back(0);
            val.push_back(0);
            invertido.push_back(invertido.back());
        ...
        if (invertido[vez] == 0)
            ...
            atual = N - val[raiz[vez]];
        if (janela.front().first == vez - K) janela.pop_front();
        ii par = ii(vez, atual);
        while (!janela.empty() && janela.back().second <= par.second)
            janela.pop_back();
