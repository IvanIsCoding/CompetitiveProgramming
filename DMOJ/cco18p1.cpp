...
int solve(int a, int b) {
    ...
    if (a == 0 || b == 0) return 0;
    if ((vetor[0][a] > vetor[1][b] && matriz[0][a] == 1 && matriz[1][b] == 0) ||
        (vetor[0][a] < vetor[1][b] && matriz[0][a] == 0 && matriz[1][b] == 1)) {
        return dp[a][b] = max(solve(a - 1, b - 1) + vetor[0][a] + vetor[1][b],
                              max(solve(a - 1, b), solve(a, b - 1)));
    } else {
