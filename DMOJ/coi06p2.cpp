...
int n, m, e1[MAXM], e2[MAXM], isbridge[MAXM], low[MAXN], num[MAXN], dfsCount,
    treeComp, bridgeTreeId[MAXN], biconnectedId[MAXN], articulation[MAXN],
    blockComp;
...
int path(int u, int v, int g1, int g2) {
    ...
    if (LCA(g1, ancestor) == ancestor &&
        (LCA(g2, v) == g2 || LCA(g2, u) == g2)) {
        return 0;
    }
    return 1;
}
...
int query2(int pos, int left, int right, int i, int j) {
    ...
    int mid = (left + right) / 2;
    if (j <= mid)
        return query2(2 * pos, left, mid, i, j);
    else if (i >= mid + 1)
        return query2(2 * pos + 1, mid + 1, right, i, j);
    return join2(query2(2 * pos, left, mid, i, j),
                 query2(2 * pos + 1, mid + 1, right, i, j));
}
...
int main() {
    ...
    for (int i = 1; i <= blockComp; i++) {
        ...
        blockCut[i].erase(unique(blockCut[i].begin(), blockCut[i].end()),
                          blockCut[i].end());
    }
    dfs_bridge(1, -1, -1);
    dfs_tree(1, -1);
