// Ivan Carvalho
// Solution to https://dmoj.ca/problem/mnyc17p6
#include <bits/stdc++.h>
using namespace std;
const int MAXN = 1e5 + 10;
const int BUCKET = 2155;
struct que {
    int id, l, r, t;
};
struct upd {
...
int divisao[MAXN], mo_left, mo_right, mo_time, n, q, freq[2 * MAXN],
    exibir[MAXN], vetor[MAXN], resp;
vector<que> Q;
vector<upd> U;
...
int converte(int xy) {
    if (!conversao.count(xy)) {
        conversao[xy] = conversao.size() + 1;
    }
    return conversao[xy];
}
int main() {
