...
ll X_p[MAXN], Y_p[MAXN], ini[MAXN], meio[MAXN], fim[MAXN], resp[MAXN],
    somatorio;
...
void processa() {
    ...
    for (int i = 1; i <= N; i++) {
        ...
        ll x0 = X_p[i] - meio[i];
        ll x1 = X_p[i] + meio[i];
        y0 = lower_bound(compy.begin(), compy.end(), y0) - compy.begin() + 1;
        y1 = prev(upper_bound(compy.begin(), compy.end(), y1)) - compy.begin() +
             1;
    ...
    // Varrendo
    for (int i = 0; i < T; i++) {
        ll x = get<0>(sweep[i]), y0 = get<1>(sweep[i]), y1 = get<2>(sweep[i]);
...
int main() {
    ...
    for (int i = 1; i <= M; i++) {
        Y_b[i] =
            lower_bound(compy.begin(), compy.end(), Y_b[i]) - compy.begin() + 1;
