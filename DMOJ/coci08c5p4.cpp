...
int main() {
    ...
    for (int dia = 1; dia <= H; dia++) {
        if (visto[bitmask_hoje] != 0) {
            ...
            H = H - dia + 1;
            total_soma +=
                1LL * (soma_acumulada[dia - 1] - soma_acumulada[comeco - 1]) *
                (H / periodo);
            int resto = H % periodo;
            if (resto != 0)
                total_soma += soma_acumulada[comeco + resto - 1] -
                              soma_acumulada[comeco - 1];
            break;
        }
        ...
        for (int i = 0; i < N; i++) {
            int recebeu =
                __builtin_popcount(bitmask_hoje & mascaras[i]) +
                __builtin_popcount((bitmask_hoje ^ tudo_ligado) & mascaras[i]) *
                    2;
