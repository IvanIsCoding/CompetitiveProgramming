...
int dist(int A, int B) { return max(A - B, B - A); }

int solve(int atual, int lado) {
    if (atual == N + 2) return 0;
    if (dp[atual][lado] != -1) return dp[atual][lado];
    int pos = P[atual - 1][lado];
    int v1 = solve(atual + 1, 1) + dist(pos, P[atual][0]) +
             dist(P[atual][0], P[atual][1]);
    int v2 = solve(atual + 1, 0) + dist(pos, P[atual][1]) +
             dist(P[atual][1], P[atual][0]);
