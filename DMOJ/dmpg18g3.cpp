...
int sz[MAXN], nivel[MAXN], N, Q, ini[MAXN], fim[MAXN], vetor[MAXN], dfsPtr,
    pai[MAXN], pesado[MAXN], range_update[MAXN];
int bit[MAXN], MAXIMO;
...
void dfs_sack(int v, int p, int keep) {
    ...
    if (pesado[v]) {
        ...
        for (int i = 0; i < perguntas[v].size(); i++) {
            int id = get<0>(perguntas[v][i]), h = get<1>(perguntas[v][i]),
                tot = 0;
            tot = read(h);
            respostas[id] += tot;
        ...
        for (int u : grafo[v]) {
            ...
            for (int i = ini[u]; i <= fim[u]; i++) {
                ...
                for (int j = 0; j < perguntas[v].size(); j++) {
                    int id = get<0>(perguntas[v][j]),
                        h = get<1>(perguntas[v][j]),
                        tot = get<2>(perguntas[v][j]);
