...
ll arvore[4 * MAXN], custo[MAXN], nivel[MAXN], distancia[MAXN], resposta[MAXN],
    n, vetorzao[MAXN], P[MAXN];
...
void dfs(ll v, ll p) {
    ...
    if (nivel[v] <= 2) {
        ...
        custo[v] = custo[vetorzao[resp]] +
                   (distancia[v] - distancia[vetorzao[resp]]) * P[v];
    }
