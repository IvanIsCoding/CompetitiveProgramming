...
int last[MAXM], pai[MAXM], sz[MAXM], granderesposta, ultimot, N, ptr,
    original[MAXM];
vector<quadra> sweep;
int find(int x) {
    if (last[x] != ultimot) {
    ...
    for (int i = 1; i <= N; i++) {
        for (int j = 1; j <= N; j++) {
            if (i + 1 <= N && V[i][j] != V[i + 1][j]) {
                ...
                sweep.push_back(
                    make_tuple(v1, v2, conversao[i][j], conversao[i + 1][j]));
            }
            if (j + 1 <= N && V[i][j] != V[i][j + 1]) {
                ...
                sweep.push_back(
                    make_tuple(v1, v2, conversao[i][j], conversao[i][j + 1]));
            }
        }
    }
