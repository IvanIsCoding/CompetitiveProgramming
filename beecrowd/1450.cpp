// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1450
#include <algorithm>
#include <cstdio>
using namespace std;
const int MAXN = 15;
    ...
    for (int prox = 0; prox < n; prox++) {
        if (prox != obj && !(bitmask & (1 << prox))) {
            for (int novoestado = 0; novoestado < 6; novoestado++) {
                if (X[obj][estado] >= X[prox][novoestado] &&
                    Y[obj][estado] >= Y[prox][novoestado]) {
                    ans = max(ans, Z[prox][novoestado] +
                                       solve(prox, bitmask | (1 << prox),
                                             novoestado));
                }
            }
        }
