// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1299
#include <cstdio>
#define MAXN 52
int dx[4] = {1, -1, 0, 0};
int dy[4] = {0, 0, 1, -1};
int componente[MAXN * MAXN], matchx[MAXN][MAXN], matchy[MAXN][MAXN],
    processado[MAXN][MAXN], componentes, marcado[MAXN][MAXN], n, m,
    meucomponente[MAXN][MAXN], iteracao;
char mapa[MAXN][MAXN];
...
int augmenting_path(int lx, int ly) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (matchx[rx][ry] == -1 ||
            augmenting_path(matchx[rx][ry], matchy[rx][ry])) {
            matchx[rx][ry] = lx;
            matchy[rx][ry] = ly;
            return 1;
        }
...
int main() {
    while (scanf("%d %d", &n, &m) != EOF) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                if ((i + j) % 2 == 0 && !marcado[i][j]) {
                    ...
                    iteracao++;
                    componente[meucomponente[i][j]] -=
                        2 * augmenting_path(i, j);
                }
            }
        }
        int primeiro = 0;
        ...
        // for(int i=0;i<n;i++){
        //	for(int j=0;j<m;j++){
        //		if((i+j)%2 == 1 && !marcado[i][j])printf("Match[%d][%d]
        // do
        // Componente %d =
