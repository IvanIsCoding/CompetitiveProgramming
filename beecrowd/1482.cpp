// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1482
#include <algorithm>
#include <cstdio>
using namespace std;
...
void solve(int museu, int bitmask, int tempo, int acesos) {
    ...
    for (int i = 1; i < n; i++) {
        ...
        if (!(bitmask & (1 << proximo))) {
            solve(proximo, bitmask | (1 << proximo),
                  tempo + matriz[museu][proximo], acesos + 1);
            if (resp == cota) return;
        }
    }
}
bool comp(int a, int b) { return matriz[davezsort][a] < matriz[davezsort][b]; }
bool comp2(int a, int b) { return custo[a] < custo[b]; }
int main() {
    while (scanf("%d", &n) && n) {
        resp = 0;
        cota = 0;
        ...
        for (int k = 0; k < n; k++) {
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++) {
                    matriz[i][j] =
                        min(matriz[i][j], matriz[i][k] + matriz[k][j]);
                }
            }
        }
        for (int i = 0; i < n; i++) {
