// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1499
#include <algorithm>
#include <cstdio>
#include <cstring>
#include <vector>
...
int maior_quadradonia[MAXN], maior_nlogonia[MAXN], aux_nlogonia,
    aux_quadradonia, processado[MAXN], diametro_nlogonia, diametro_quadradonia;
...
int main() {
    while (scanf("%d %d", &N, &Q) != EOF) {
        ...
        for (vector<ii>::iterator it1 = conjunto_quadradonia.begin();
             it1 != conjunto_quadradonia.end(); it1++) {
            for (vector<ii>::iterator it2 = conjunto_nlogonia.begin();
                 it2 != conjunto_nlogonia.end(); it2++) {
                resp += max((*it1).first + (*it2).first + 1, (ll)diametro) *
