// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1302
#include <algorithm>
#include <cmath>
#include <cstdio>
#include <cstdlib>
...
int vaipara[MAXN], n, q, pai[MAXN], paisubarvore[MAXN], ancestral[MAXN],
    superancestral[MAXN], nivel[MAXN], tamanho[MAXN], valor[MAXN], segmento;
int find(int x) {
    if (x == pai[x]) return x;
    ...
    while (scanf("%d", &n) != EOF) {
        ...
        while (q--) {
            ...
            if (paisubarvore[u] == paisubarvore[v]) {
                ...
                int delta =
                    abs(valor[paisubarvore[u]] - valor[paisubarvore[v]]);
