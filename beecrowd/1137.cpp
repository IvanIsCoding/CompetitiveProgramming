// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1137
#include <algorithm>
#include <iostream>
#define MAXN 201
using namespace std;
typedef long long ll;
ll X[MAXN], Y[MAXN], squared[MAXN], resp, n, sqy[MAXN][MAXN], sqx[MAXN][MAXN],
    xy[MAXN][MAXN];
void addCircle(ll A, ll B, ll C) {
    ll cof1 = X[A] * (Y[B] - Y[C]) - Y[A] * (X[B] - X[C]) + xy[B][C];
    if (cof1 == 0) return;
    ll cof2 = squared[A] * (Y[B] - Y[C]) - Y[A] * (squared[B] - squared[C]) +
              sqy[B][C];
    ll cof3 = squared[A] * (X[B] - X[C]) - X[A] * (squared[B] - squared[C]) +
              sqx[B][C];
