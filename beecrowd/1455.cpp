// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1455
#include <algorithm>
#include <cmath>
#include <cstdio>
#include <vector>
...
void addCircle(int A, int B, int C, int n) {
    double cof1 =
        X[A] * (Y[B] - Y[C]) - Y[A] * (X[B] - X[C]) + X[B] * Y[C] - Y[B] * X[C];
    double cof2 = squared[A] * (Y[B] - Y[C]) -
                  Y[A] * (squared[B] - squared[C]) + squared[B] * Y[C] -
                  Y[B] * squared[C];
    double cof3 = squared[A] * (X[B] - X[C]) -
                  X[A] * (squared[B] - squared[C]) + squared[B] * X[C] -
                  X[B] * squared[C];
    double cof4 = squared[A] * (X[B] * Y[C] - Y[B] * X[C]) -
                  X[A] * (squared[B] * Y[C] - Y[B] * squared[C]) +
                  Y[A] * (squared[B] * X[C] - X[B] * squared[C]);
