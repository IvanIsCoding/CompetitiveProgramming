// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1356
#include <cstdio>
#include <cstring>
#define MAXN 100010
#define LSOne(S) (S & -S)
...
ll inv(ll val) { return fast_expo(val % MODULO, MODULO - 2LL); }
...
int main() {
    while (scanf("%lld %lld %lld %lld", &B, &MODULO, &L, &N) &&
           (B || MODULO || L || N)) {
        ...
        // for(ll i=1;i<=L;i++){
        //	printf("%lld ",(((sum(i) - sum(i-1)) % MODULO) + MODULO) %
        // MODULO);
        // }
        // printf("\n");
        printf("-\n");
