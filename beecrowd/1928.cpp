// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1928
#include <algorithm>
#include <cstdio>
#include <vector>
...
ll resposta;
int n, c[MAXN], par[MAXN], pai[MAXN], nivel[MAXN], ancestral[MAXN][MAXL],
    visitado[MAXN];
...
int LCA(int x, int y) {
    ...
    for (int i = MAXL - 1; i >= 0; i--)
        if (nivel[x] - (1 << i) >= nivel[y]) x = ancestral[x][i];
    if (x == y) return x;
    for (int i = MAXL - 1; i >= 0; i--) {
