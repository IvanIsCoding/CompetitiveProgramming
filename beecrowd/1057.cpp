// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1057
#include <cstdio>
#include <queue>
#define MAXN 9
using namespace std;
...
struct ponto {
    ...
    ponto(int _x1, int _y1, int _x2, int _y2, int _x3, int _y3, int _percorrido)
        : x1(_x1),
          y1(_y1),
          x2(_x2),
          y2(_y2),
...
int main() {
    ...
    for (int tc = 1; tc <= TC; tc++) {
        ...
        while (!bfs.empty()) {
            ...
            if (mapa[d.x1][d.y1] == 'X' && mapa[d.x2][d.y2] == 'X' &&
                mapa[d.x3][d.y3] == 'X') {
                possivel = 1;
                printf("Case %d: %d\n", tc, d.percorrido);
                break;
            ...
            for (int i = 0; i < 4; i++) {
                ponto agora(d.x1 + (cord(d.x1 + dx[i], d.y1 + dy[i]) &&
                                            mapa[d.x1 + dx[i]][d.y1] != '#'
                                        ? dx[i]
                                        : 0),
                            d.y1 + (cord(d.x1 + dx[i], d.y1 + dy[i]) &&
                                            mapa[d.x1][d.y1 + dy[i]] != '#'
                                        ? dy[i]
                                        : 0),
                            d.x2 + (cord(d.x2 + dx[i], d.y2 + dy[i]) &&
                                            mapa[d.x2 + dx[i]][d.y2] != '#'
                                        ? dx[i]
                                        : 0),
                            d.y2 + (cord(d.x2 + dx[i], d.y2 + dy[i]) &&
                                            mapa[d.x2][d.y2 + dy[i]] != '#'
                                        ? dy[i]
                                        : 0),
                            d.x3 + (cord(d.x3 + dx[i], d.y3 + dy[i]) &&
                                            mapa[d.x3 + dx[i]][d.y3] != '#'
                                        ? dx[i]
                                        : 0),
                            d.y3 + (cord(d.x3 + dx[i], d.y3 + dy[i]) &&
                                            mapa[d.x3][d.y3 + dy[i]] != '#'
                                        ? dy[i]
                                        : 0),
                            d.percorrido + 1);
                if (valido(agora))
                    ...
                    if (agora.x1 == agora.x2 && agora.y1 == agora.y2) {
                        agora.x1 -= (cord(d.x1 + dx[i], d.y1 + dy[i]) &&
                                             mapa[d.x1 + dx[i]][d.y1] != '#'
                                         ? dx[i]
                                         : 0);
                        agora.y1 -= (cord(d.x1 + dx[i], d.y1 + dy[i]) &&
                                             mapa[d.x1][d.y1 + dy[i]] != '#'
                                         ? dy[i]
                                         : 0);
                        ...
                        agora.y1 += (cord(d.x1 + dx[i], d.y1 + dy[i]) &&
                                         ...
                                         ? dy[i]
                                         : 0);
                        agora.x2 -= (cord(d.x2 + dx[i], d.y2 + dy[i]) &&
                                             mapa[d.x2 + dx[i]][d.y2] != '#'
                                         ? dx[i]
                                         : 0);
                        agora.y2 -= (cord(d.x2 + dx[i], d.y2 + dy[i]) &&
                                             mapa[d.x2][d.y2 + dy[i]] != '#'
                                         ? dy[i]
                                         : 0);
                        if (valido(agora)) bfs.push(agora);
                        ...
                        agora.y1 += (cord(d.x1 + dx[i], d.y1 + dy[i]) &&
                                         ...
                                         ? dy[i]
                                         : 0);
                        agora.x3 -= (cord(d.x3 + dx[i], d.y3 + dy[i]) &&
                                             mapa[d.x3 + dx[i]][d.y3] != '#'
                                         ? dx[i]
                                         : 0);
                        agora.y3 -= (cord(d.x3 + dx[i], d.y3 + dy[i]) &&
                                             mapa[d.x3][d.y3 + dy[i]] != '#'
                                         ? dy[i]
                                         : 0);
                        if (valido(agora)) bfs.push(agora);
