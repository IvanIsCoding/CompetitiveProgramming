// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/2092
#include <algorithm>
#include <cstdio>
#include <set>
#define MAXN 100010
...
int main() {
    ...
    while (m--) {
        ...
        if (conjunto[time].find(casa) != conjunto[time].begin())
            for (set<int>::iterator it = conjunto[time].find(casa);;) {
                it--;
                int davez = *it;
                int temp = casa;
                if (temp == davez) break;
                ...
                // printf("CASA %d DAVEZ %d CCW %d CW
                // %d\n",temp,davez,antihorario,horario);
            }
        if (!achamosantihorario)
            for (set<int>::iterator it = conjunto[time].end();;) {
                it--;
                int davez = *it;
                int temp = casa;
                if (temp == davez) break;
        ...
        if (!achamoshorario)
            for (set<int>::iterator it = conjunto[time].begin();
                 it != conjunto[time].find(casa); it++) {
                int davez = *it;
                int temp = casa;
                if (temp == davez) break;
                bool flag = false;
