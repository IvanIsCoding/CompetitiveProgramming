// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1529
#include <cstdio>
#include <queue>
#include <vector>
#define MP make_pair
...
int main() {
    while (scanf("%d %d %d", &n, &m, &t) && (n || m || t)) {
        ...
        while (!Dijkstra.empty()) {
            ...
            Dijkstra.pop();
            int gasto = davez.first, tanque = davez.second.first,
            ...
            for (int i = 0; i < grafo[cidade].size(); i++) {
                int proximo = grafo[cidade][i].first,
                    proxdist = grafo[cidade][i].second;
                if (proxdist <= tanque) {
