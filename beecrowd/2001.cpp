// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/2001
#include <algorithm>
#include <cstdio>
#include <vector>
...
matrix mult(matrix A, matrix B) {
    ...
    REP(i, K)
    REP(j, K)
    REP(k, K) C.mat[i][j] = (C.mat[i][j] + A.mat[i][k] * B.mat[k][j]);
...
int main() {
    ...
    while (scanf("%lld %lld", &K, &n) != EOF) {
        ...
        ll exibir = 0;
        REP(i, K) { exibir = (exibir + novaresp.mat[1][i + 1] * A[i]) % MOD; }
        novoexibir += exibir;
        novoexibir %= MOD;
        printf("%lld %lld\n", exibir, novoexibir);
    }
