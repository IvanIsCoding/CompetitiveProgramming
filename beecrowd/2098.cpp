// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/2098
#include <cstdio>
#include <queue>
#define MP make_pair
using namespace std;
...
int dx[4] = {1, -1, 0, 0};
int dy[4] = {0, 0, 1, -1};
inline int valido(int x, int y) { return x >= 1 && x <= r && y >= 1 && y <= c; }
int func(int altura) {
    iteracao++;
    queue<iii> bfs;
    ...
    while (!bfs.empty()) {
        iii davez = bfs.front();
        bfs.pop();
        int percorrido = davez.first, x = davez.second.first,
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (valido(nx, ny) && processado[nx][ny] != iteracao)
