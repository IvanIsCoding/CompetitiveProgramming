// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1970
#include <algorithm>
#include <cstdio>
#include <cstring>
#define MAXN 101
...
int func(int n, int aguenta1, int aguenta2, int aguenta3) {
    ...
    if (dp[n][aguenta1][aguenta2][aguenta3] != -1)
        return dp[n][aguenta1][aguenta2][aguenta3];
    int nao_coloca = func(n - 1, aguenta1, aguenta2, aguenta3), coloca1 = 0,
        coloca2 = 0, coloca3 = 0;
    if (peso[n] <= aguenta1) {
    ...
    return dp[n][aguenta1][aguenta2][aguenta3] =
               max(max(max(coloca1, coloca2), coloca3), nao_coloca);
}
int main() {
