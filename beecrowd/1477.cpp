// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1477
#include <cstdio>
#include <cstring>
#define MAXN 100010
...
int query_homem(int pos, int left, int right, int i, int j) {
    ...
    int mid = (left + right) / 2;
    return query_homem(2 * pos, left, mid, i, j) +
           query_homem(2 * pos + 1, mid + 1, right, i, j);
}
int query_elefante(int pos, int left, int right, int i, int j) {
    ...
    int mid = (left + right) / 2;
    return query_elefante(2 * pos, left, mid, i, j) +
           query_elefante(2 * pos + 1, mid + 1, right, i, j);
}
int query_rato(int pos, int left, int right, int i, int j) {
    ...
    int mid = (left + right) / 2;
    return query_rato(2 * pos, left, mid, i, j) +
           query_rato(2 * pos + 1, mid + 1, right, i, j);
}
int main() {
    int n, q;
    while (scanf("%d %d", &n, &q) != EOF) {
        ...
        while (q--) {
            ...
            if (op == 'C')
                printf("%d %d %d\n", query_homem(1, 1, n, a, b),
                       query_elefante(1, 1, n, a, b),
                       query_rato(1, 1, n, a, b));
        }
        printf("\n");
    }
    return 0;
