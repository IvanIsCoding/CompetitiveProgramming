// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1810
#include <algorithm>
#include <cstdio>
#include <cstring>
#define MAXN 181
...
int solve(int dia, int ultimo) {
    ...
    if (ultimo == dia - 1) {
        ...
        custodiario =
            h[dia - 1] * (demandatotal[ultimo] - demandatotal[dia - 1]);
        melhor = min(melhor, solve(dia + 1, ultimo) + custodiario);
        for (int proximo = ultimo + 1; proximo <= n; proximo++) {
            int delta =
                (demandatotal[proximo] - demandatotal[ultimo]) * p[dia] +
                f[dia];
            melhor = min(melhor, solve(dia + 1, proximo) + custodiario + delta);
        }
    }
