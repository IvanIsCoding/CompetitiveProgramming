// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1513
#include <algorithm>
#include <cstdio>
#include <queue>
#define MAXN 101
...
int conversao[MAXN][MAXN], n, m, k, dist[MAXN][MAXN], processado[MAXN][MAXN],
    dp[MAXK][MAXL];
...
void bfs(int v, int x, int y) {
    ...
    while (!fila.empty()) {
        ...
        fila.pop();
        int percorrido = davez.first, cordx = davez.second.first,
            cordy = davez.second.second;
        if (cordx < 0 || cordy < 0 || cordx > n || cordy > m ||
            entrada[cordx][cordy] == '#')
            continue;
