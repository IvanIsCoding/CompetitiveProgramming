// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1539
#include <algorithm>
#include <cmath>
#include <cstdio>
#define MAXN 110
...
int main() {
    while (scanf("%d", &n) && n != 0) {
        ...
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= n; j++) {
                double distance = sqrt(pow(coordx[i] - coordx[j], 2) +
                                       pow(coordy[i] - coordy[j], 2));
                if (distance <= raio[i])
