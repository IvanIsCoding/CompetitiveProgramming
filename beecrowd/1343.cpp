// Ivan Carvalho
// Solution to https://www.beecrowd.com.br/judge/problems/view/1343
#include <cstdio>
#include <queue>
#include <set>
#include <vector>
...
inline int vencedor(vi pecas) { return pecas.size() == 1; }
...
int main() {
    ...
    for (int i = 1; i < MAXL; i++) {
        for (int j = 1; j < MAXL; j++) {
            int id = matriz[i][j];
            for (int k = 0; k < 8; k++) {
                if (valido(i + dx[k], j + dy[k]))
                    grafo[id].PB(matriz[i + dx[k]][j + dy[k]]);
            }
        }
    }
