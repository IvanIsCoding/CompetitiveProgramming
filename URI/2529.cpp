// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/2529
#include <cstdio>
#include <cstdlib>
#include <algorithm>
using namespace std;
...
struct node{
	...
	int rev,size,prior;
	int val,total;
	node(int val) : val(val),total(val),prior(rand()),l(NULL),r(NULL),size(1),rev(0){}
};
...
void operation(pnode t){if(t) t->total = tot(t->l) + t->val + tot(t->r);}
void push(pnode &t){
	if(t && t->rev){
		t->rev = 0;
		swap(t->l,t->r);
