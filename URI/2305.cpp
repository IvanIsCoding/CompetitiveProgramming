// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/2305
#include <cstdio>
#include <algorithm>
using namespace std;
...
int main(){
	...
	for(int linha = 1;linha + p - 1 <= n;linha++){
		for(int coluna = 1;coluna + q - 1 <=m;coluna++){
			...
			int temp = 0;
			for(int linhadavez = linha;linhadavez <= linhalimite;linhadavez++){
				temp += soma[linhadavez][colunafim] - soma[linhadavez][colunaini];
			}
			//printf("Linha %d Coluna %d Colunafim %d Colunaini %d Linhalimite %d Temp %d Resp %d\n",linha,coluna,colunafim,colunaini,linhalimite,temp,resp);
			resp = max(temp,resp);
		}
	}
	printf("%d\n",resp);
