// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/2308
#include <cstdio>
#include <algorithm>
#include <vector>
#include <queue>
...
int main(){
	...
	for(int i=1;i<=n;i++) {
		...
		while(!heap.empty()){
			...
			for(int j=0;j<(int)grafo[davez.second].size();j++) heap.push(MP(davez.first+grafo[davez.second][j].first,grafo[davez.second][j].second));
		}
	}
	for(int i=1;i<=n;i++) for(int j=1;j<=n;j++) resposta = min(resposta,matriz[i][j]+matriz[j][i]);
	printf("%d\n",resposta);
	return 0;
}
