// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/2186
#include <cstdio>
#include <cstdlib>
#include <algorithm>
#include <vector>
...
struct node{
	...
	int puro;
	vector<int> total;
	node(int puro) : puro(puro),size(1),prior(rand()),rev(0),l(NULL),r(NULL){total = modelo[puro];} 
};
...
inline void operation(pnode t){if(t) for(int c=0;c<ALPHABET_SIZE;c++) t->total[c] = tot(t->l,c) + (t->puro == c) + tot(t->r,c);}
void push(pnode &t){
	if(t && t->rev){
		t->rev = 0;
		swap(t->l,t->r);
