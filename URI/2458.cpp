// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/2458
#include <cstdio>
using namespace std;
#define MAXN 550
int n;
...
int dfs(int x,int y){
	...
	int retornar = 1;
	visitado[x][y] = true;
	if (elegivel(x-1,y) && matriz[x-1][y] == 'V') retornar += dfs(x-1,y);
	if (elegivel(x+1,y) && matriz[x+1][y] == 'A') retornar += dfs(x+1,y);
	if (elegivel(x,y-1) && matriz[x][y-1] == '>') retornar += dfs(x,y-1);
	if (elegivel(x,y+1) && matriz[x][y+1] == '<') retornar += dfs(x,y+1);
	return retornar;
}
int main(){
