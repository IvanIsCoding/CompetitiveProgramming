// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/2371
#include <cstdio>
#include <vector>
#include <set>
#include <map>
...
int main(){
	...
	for(it=mapa.begin();it!=mapa.end();it++){
		...
		pair<int,int> a1=make_pair(x+1,y),a2=make_pair(x-1,y),b1=make_pair(x,y+1),b2=make_pair(x,y-1);
	...
	for(int i=1;i<=contador;i++){
		if (navios.find(componente[i])!=navios.end()) navios[componente[i]]++;
		else navios[componente[i]]=1;
	}
	scanf("%d",&k);
	for(int i=0;i<k;i++){
		int a,b;
