// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/2558
#include <cstdio>
#include <cstring>
#include <algorithm>
#include <queue>
...
int check(vector<int> vetorzao){
	...
	return dist[0][vetorzao[0]] + percorrido + dist[vetorzao[vetorzao.size()-1]][N+1];
}
void func(int bitmask){
	vector<int> davez;
	for(int i=0;i<N;i++) if(bitmask & (1 << i)) davez.push_back(i+1);
	...
	while(scanf("%d %d",&N,&M) != EOF){
		...
		for(int vez=0;vez<=N+1;vez++){
			...
			while(!bfs.empty()){
				iii temp = bfs.front();
				bfs.pop();
				int x = temp.second.first, y = temp.second.second, percorrido = temp.first;
				if(S[x][y] <= percorrido) continue;
				S[x][y] = percorrido;
				for(int i=0;i<4;i++){
					int nx = x + dx[i];
					int ny = y + dy[i];
					if(valido(nx,ny)) bfs.push(MP(percorrido+1,MP(nx,ny)));
				}
			}
			for(int novovez = 0;novovez <= N+1;novovez++) dist[vez][novovez] = S[posx[novovez]][posy[novovez]];
		}
		for(int i = 0;i<(1<<N);i++){
			func(i);
		}
