// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/2700
#include <bits/stdc++.h>
#define LSOne(S) (S & (-S))
using namespace std;
...
int main(){
	cin.tie(0);cout.tie();ios_base::sync_with_stdio(0);
	cin >> N;
	for(int i = 0;i<N;i++){
		cin >> e1[i] >> e2[i] >> e3[i];
		comp.push_back(e2[i]);
	...
	for(int i = 0;i<N;i++){
		if(!agoravai.empty() && get<0>(agoravai.back()) == get<0>(sweep[i]) && get<1>(agoravai.back()) == get<1>(sweep[i]) ){
