// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/2184
#include <cstdio>
#include <algorithm>
#include <vector>
#include <cmath>
...
int pai[MAXN],superpai[MAXN],ligapai[MAXN],ligasuperpai[MAXN],nivel[MAXN],segmento,referencia;
int n,k,q;
...
void dfs_superpai(int x,int p){
	...
	for(int i=0;i<grafo[x].size();i++){
		...
		if(!superpai[v]){
			superpai[v] = p;
			if(p != x) ligasuperpai[v] = max(ligasuperpai[x],peso);
			else ligasuperpai[v] = peso;
			dfs_superpai(v,p);
		}
	}
}
