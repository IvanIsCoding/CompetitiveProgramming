// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/2081
#include <cstdio>
#include <queue>
#define MAXN 51
using namespace std;
...
int main(){
	...
	for(int instancia=1;instancia <= TC;instancia++){
		...
		while(!bfs.empty()){
			ponto d = bfs.front();
			bfs.pop();
			if(processado[d.x1][d.y1][d.x2][d.y2] == instancia || d.percorrido >= MAXN) continue;
			...
			if(d.x1 - 1 >= 0 && d.x2 - 1 >= 0 && mapa1[d.x1-1][d.y1] != 'B' && mapa2[d.x2-1][d.y2] != 'B'){
				ponto proximo;
				...
				proximo.y1 = d.y1;
				proximo.y2 = d.y2;
				if (processado[proximo.x1][proximo.y1][proximo.x2][proximo.y2] != instancia) bfs.push(proximo);
			}
			if(d.x1 + 1 < n && d.x2 + 1 < n && mapa1[d.x1+1][d.y1] != 'B' && mapa2[d.x2+1][d.y2] != 'B'){
				ponto proximo;
				proximo.percorrido = d.percorrido + 1;
				proximo.x1 = d.x1 + (mapa1[d.x1+1][d.y1] != '#')*1;
				proximo.x2 = d.x2 + (mapa2[d.x2+1][d.y2] != '#')*1;
			...
			if(d.y1 - 1 >= 0 && d.y2 - 1 >= 0 && mapa1[d.x1][d.y1-1] != 'B' && mapa2[d.x2][d.y2-1] != 'B'){
				ponto proximo;
				proximo.percorrido = d.percorrido + 1;
			...
			if(d.y1 + 1 < m && d.y2 + 1 < m && mapa1[d.x1][d.y1+1] != 'B' && mapa2[d.x2][d.y2+1] != 'B'){
				ponto proximo;
				proximo.percorrido = d.percorrido + 1;
				proximo.y1 = d.y1 + (mapa1[d.x1][d.y1+1] != '#')*1;
				proximo.y2 = d.y2 + (mapa2[d.x2][d.y2+1] != '#')*1;
