// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/2330
#include <cstdio>
#include <queue>
#include <vector>
#include <algorithm>
...
int main(){
	...
	while(l--){
		...
		fila.pop();
		fila.push(make_pair(davez.first+duracao,make_pair(davez.second.first,davez.second.second+1)));
	}
	for(int i=1;i<=n;i++){
		pair<int, pair<int,int> > davez = fila.top();
		fila.pop();
