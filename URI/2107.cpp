...
char mesmo_semiplano(int A,int B,int C,int D,int E){
	int cof1 = Y[A]*(Z[B] - Z[C]) - Z[A]*(Y[B]-Y[C]) + Y[B]*Z[C] - Y[C]*Z[B];
	int cof2 = X[A]*(Z[B] - Z[C]) - Z[A]*(X[B]-X[C]) + X[B]*Z[C] - X[C]*Z[B];
	cof2 *= -1;
	int cof3 = X[A]*(Y[B] - Y[C]) - Y[A]*(X[B]-X[C]) + X[B]*Y[C] - X[C]*Y[B];
	int cof4 = X[A]*(Y[B]*Z[C] - Y[C]*Z[B]) - Y[A]*(X[B]*Z[C] - X[C]*Z[B]) + Z[A]*(X[B]*Y[C] - X[C]*Y[B]);
	cof4 *= -1;
