...
int main(){
	...
	while(m--){
		...
		if (tipo) grafo_multiplo[u].PB(v);
		else grafo_normal[u].PB(v);
	}
	queue<ii> fila;
	fila.push(MP(e,0));
	while(!fila.empty()){
		...
		if (d%3==0){
			...
			visitados[v] = 1;
			for(int i=0;i<grafo_multiplo[v].size();i++) fila.push(MP(grafo_multiplo[v][i],d+1));
		}
		if (d%3==1){
			if (visitados1[v]) continue;
			visitados1[v] = 1;
			for(int i=0;i<grafo_normal[v].size();i++) fila.push(MP(grafo_normal[v][i],d+1));
		}
		if (d%3==2){
