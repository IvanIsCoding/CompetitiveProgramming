...
vector<int> grafo[50001]; // Armazena o grafo, 5*10^4 porque é o tamanho da entrada (nota : tem que ter 1 de sobra)
...
int main(){
	...
	for(int i=1;i<=M;i++){ // Loop para iterar as arestas 
		...
		//O u entre [] indica que são os vizinhos de U. O v entre () indica que v é adicinado
	}
	for(int vertice = 1;vertice<=N;vertice++){
		...
		if(!ok[vertice]){
			...
			// push (não push_back, cuidado)  : adiciona um vértice no final da fila
			// pop : tira um vértice do início da fila
			// front : vê quem está na frente da fila
			// Uma condição adicional é checar se ela está vazia , empty()
			// A negação de empty indica que a fila não está vazia
			...
			while(!fila.empty()){ // Enquanto a fila não estiver vazia
				...
				if(!ok[u]){// Se ele não estiver ok
					ok[u] = 1; // Ele fica ok
					for(int i=0;i<grafo[u].size();i++){// Para todo i menor que o tamanho da vizinhança de U
						// Pego o elemento que está no índice i do vetor
						// size é o tamnho do grafo[u]
						int v = grafo[u][i];//Vizinho v que está no indíce I do vizinho de U
