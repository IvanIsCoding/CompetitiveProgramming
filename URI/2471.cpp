// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/2471
#include <cstdio>
#define MAXN 52
...
int main(){
	...
	for(int coluna = 1;coluna <= n;coluna++){
		for(int linha = 1;linha <=n;linha++){
			acumulada[linha][coluna] = acumulada[linha-1][coluna] + matriz[linha][coluna];
		}
	}
	...
	for(int i=1;i<=n;i++){
		for(int j=1;j<=n;j++){
			if(soma[i][n] != escolhida && acumulada[n][j] != escolhida){
				printf("%d %d\n",(2*escolhida - acumulada[n][j] - soma[i][n])/2 + matriz[i][j],matriz[i][j]);
			}
		}
	}
	return 0;
