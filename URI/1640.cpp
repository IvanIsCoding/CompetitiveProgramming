// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/1640
#include <cstdio>
#include <queue>
#define MP make_pair
using namespace std;
...
int main(){
	while(scanf("%d",&n) && n){
		...
		while(!Dijkstra.empty()){
			...
			Dijkstra.pop();
			int hoteis = davez.first, minutos = davez.second.first, cidade = davez.second.second;
			if(cidade == n){
				printf("%d\n",hoteis);
				possivel = 1;
				break;
			...
			for(int i=0;i<grafo[cidade].size();i++){
				int proxima = grafo[cidade][i].first, tempo = grafo[cidade][i].second;
