// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/1752
#include <cstdio>
#include <cstdlib>
#include <algorithm>
#include <set>
...
const int MAXN = 501;
const int INF = 1e9;
int V[MAXN][MAXN],R[MAXN][MAXN],C[MAXN][MAXN],r,c,n,X[MAXN],Y[MAXN],xi,yi,xf,yf,bit[MAXN][MAXN];
set<i4> Dijkstra;
...
int main(){
	...
	for(int vez = 1; vez + 1 <= n;vez++){
		...
		while(!Dijkstra.empty()){
			...
			int percorrido = davez.first, x = davez.second.second.first, y = davez.second.second.second;
