// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/2078
#include <cstdio>
#include <cstring>
#include <algorithm>
#include <vector>
...
int main(){
	...
	while(TC--){
		...
		for(int i=0;i<n;i++){
			...
			int xi,yi;
			scanf("%d %d",&xi,&yi);
			// giramos 45 graus e fazemos a translacao no x para nao ficar negativo e no y para nao ficar nulo
		...
		for(int i=0;i<n;i++){
			...
			resposta[vetorzao[i].id] = query(vetorzao[i].y - d, vetorzao[i].y + d);
		}
		for(int i=0;i<n;i++){
