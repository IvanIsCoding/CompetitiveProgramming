// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/2560
#include <cstdio>
#include <algorithm>
#include <deque>
#define MP make_pair
...
int main(){
	while(scanf("%d %d",&n,&b) != EOF){
		...
		for(int i =1;i<=b;i++){
			soma += vetor[i];
			while(!maximo.empty() && maximo.back().second <= vetor[i]) maximo.pop_back();
			while(!minimo.empty() && minimo.back().second >= vetor[i]) minimo.pop_back();
