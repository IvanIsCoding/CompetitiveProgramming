// Ivan Carvalho
// Solution to https://www.urionlinejudge.com.br/judge/problems/view/2398
#include <cstdio>
#include <cstdlib>
#include <algorithm>
#include <vector>
	...
	for(int i=0;i<n;i++){
		for(int j=0;j<n;j++){
			...
			for(int k=0;k<pares.size();k++){
				...
				if (abs(i-par.second.first)+abs(j-par.second.second)!=par.first){
					pode_ser = 0;
					break;
				}
			}
